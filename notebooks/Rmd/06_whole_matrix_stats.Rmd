...
```{r}
annotated_graph_edges %>% pull(from_to_type) %>% unique()
```
...
edgewise_vertex_type_delta_summary <- FC_boots %>%
  ...
  mutate(sig_range = ((min < 0 & max < 0 ) | (min > 0 & max > 0)),
         sig_05CI = ((lower < 0 & upper < 0 ) | (lower > 0 & upper > 0))) %>%
  select(-lower, -upper) %>%
  pivot_wider(id_cols = c("to", "from"),
              names_from = comparison_type,
              values_from = c(mean_val:sig_05CI))
...
```{r} 
edgewise_vertex_type_delta_summary %>%
  filter(sig_range_pvertex) %>%
  filter(sig_05CI_pvertex_minus_tvertex) %>%
  select(to, from, mean_val_tvertex, mean_val_pvertex)

```
```{r} 
edgewise_vertex_type_delta_summary %>%
  filter(sig_range_tvertex) %>%
  filter(sig_05CI_tvertex_minus_tvolume) %>%
  select(to, from, mean_val_tvertex, mean_val_tvolume, mean_val_tvertex_minus_tvolume)
