...
calc_DX_cohenD <- function(df) { 
m1 <- lm(corZ ~ Age_match_pt + Sex + fd_mean_match_pt + Scanner, data = df)
result <-df %>% 
  add_residuals(m1) %>%
  cohen.d(resid ~ DX, data = .) %>% 
  .$estimate
return(result)
}
```
...
```{r}
results_pheno_plus_DXd <- results_pheno %>%
...
DX_lm_model_full <- results_pheno %>%
  ...
  mutate(corZ = weight) %>%
  group_by(vertex_type, to, from) %>%
  do(tidy(lm(corZ ~ DX + Age_match_pt + Sex + fd_mean_match_pt + Scanner,.))) 
```
...
DX_lm_model_DXcohenD <- DX_lm_model %>%
  filter(term == "DXSSD") %>%
  inner_join(results_pheno_plus_DXd, by = c("vertex_type", "to", "from")) %>%
...
Fz_DXweigths %>% 
  ...
  summarise(mean_weight = mean(edge_FC_weight)) %>%
  ggplot(aes(y = abs(mean_weight), x = vertex_type, color = edge_group)) +
    geom_point() +
...
```
```{r}
write_csv(Fz_DXweigths_pvertex, file.path(output_base, "all_clinicalplusqa_group", "weighted_subject_FC_weigths", "SSD4cohorts_DXtweigths_pvertex.csv"))
write_csv(Fz_DXweigths_tvertex, file.path(output_base, "all_clinicalplusqa_group", "weighted_subject_FC_weigths", "SSD4cohorts_DXtweigths_tvertex.csv"))
write_csv(Fz_DXweigths_tvolume, file.path(output_base, "all_clinicalplusqa_group", "weighted_subject_FC_weigths", "SSD4cohorts_DXtweigths_tvolume.csv"))
```
...
calc_subjects_wFC_score <- function(subject_edges_df, edge_weights) {
  ...
  summarise(wFC_score = mean(value, w = edge_FC_weight),
...
groupwise_weights <- tibble(vertex_type = c("pvertex", "tvertex", "tvolume")) %>%
  mutate(result = map(vertex_type, 
                      ~ define_vertex_hypohyper_weights(DX_lm_model_DXcohenD, .x, cohenD_threshold))) %>%
  unnest()
```
