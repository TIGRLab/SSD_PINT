...
#### This reads all the timerseries and correlates them

Write a func_base and outputprefix cols into the pheno file for the file reading step

tester for the timeseries reading scripts
...
This was also pre-run - reads all timeseries and saves to a rds intermidiate file
...
data_for_harmonize_pvertex <- tmp %>%
  select(all_of(pheno$subject_dataset))

pvertex_harmonized <- neuroCombat(dat=data_for_harmonize_pvertex , batch=batch, mod=mod)


data_for_harmonize_tvertex <- all_corZ_results %>%
  mutate(subject_dataset = str_c(subject, "_", dataset)) %>%
  unnest(the_corrs) %>%
...
tvertex_harmonized <- neuroCombat(dat=data_for_harmonize_tvertex, 
                                  batch=batch, mod=mod)


data_for_harmonize_tvolume <- all_corZ_results %>%
  mutate(subject_dataset = str_c(subject, "_", dataset)) %>%
  unnest(the_corrs) %>%
...
tvolume_harmonized <- neuroCombat(dat=data_for_harmonize_tvolume, batch=batch, mod=mod)


combat_data_corZ <- bind_rows(
  pvertex = bind_cols(the_edges, as_tibble(pvertex_harmonized$dat.combat)),
  tvertex = bind_cols(the_edges, as_tibble(tvertex_harmonized$dat.combat)),
  tvolume = bind_cols(the_edges, as_tibble(tvolume_harmonized$dat.combat)),
  .id = "vertex_type")

saveRDS(combat_data_corZ, file = file.path(output_base, "all_clinicalplusqa_group", "Rdata_cache", "06_wholebrain_FC_PINTcombat_cache.rds"))
```
...
saveRDS(roiwise_boot_cohenDDX_resultse, file = file.path(output_base, "all_clinicalplusqa_group", "Rdata_cache", "06_wholebrain_FC_PINTcombat_roi_DXboots_cache.rds"))

```

This ended up being too much for one node so we needed to use the HPC for the roiwise permutations

```{sh eval=FALSE}
ssh hawking
# meant to be run from the base of the project
# cd ~/code/SSD_PINT
sbatch --array=0-99 ./code/bin/submit_PINTFC_DXeffect_10permutations.sh
```

```{r}
FC_boots <- list.files(path = file.path(output_base, "all_clinicalplusqa_group", "Rdata_cache","06_PINTFC_boots"), pattern = "*.rds", full.names = TRUE ) %>%
  map_dfr(readRDS)
```
...
calculate_mean_ci <- function(data) {
  ...
  result <- sprintf("%.2f [%.2f; %.2f]", mean_val, lower, upper)
...
calc_DX_reproducible_edgewise <- function(this_vertex_type) {
matrix_data <- FC_boots %>%
  pivot_wider(id_cols = all_of(c("to", "from")),
              names_from = "i",
              values_from = this_vertex_type,
              names_prefix = "i") %>%
  select(starts_with("i")) %>%
  cor() 

result <- calculate_mean_ci(matrix_data[upper.tri(matrix_data)])
   return(result)
}

print(str_c("pvertex: ",calc_DX_reproducible_edgewise("pvertex")))
print(str_c("tvertex: ",calc_DX_reproducible_edgewise("tvertex")))
print(str_c("tvolume: ",calc_DX_reproducible_edgewise("tvolume")))
```
[1] "pvertex: 0.78 [0.72; 0.84]*"
[1] "tvertex: 0.78 [0.72; 0.83]*"
[1] "tvolume: 0.75 [0.68; 0.81]*"

```{r}

annotated_graph_edges <- FC_boots %>%
  ungroup() %>%
  select(to, from) %>%
  distinct() %>%
...
print("Cortical Cortical correlation across boots")
print(str_c("pvertex: ",calc_DX_reproducible_edgewise("pvertex")))
print(str_c("tvertex: ",calc_DX_reproducible_edgewise("tvertex")))
print(str_c("tvolume: ",calc_DX_reproducible_edgewise("tvolume")))
```
[1] "Cortical Cortical correlation across boots"
[1] "pvertex: 0.68 [0.58; 0.76]*"
[1] "tvertex: 0.69 [0.61; 0.75]*"
[1] "tvolume: 0.62 [0.53; 0.70]*"
...
matrix_data <- FC_boots %>%
  inner_join(annotated_graph_edges, by = c("to", "from")) %>%
  filter(from_to_type == "SubCort_Cort") %>%
  pivot_wider(id_cols = c("to", "from"),
              names_from = "i",
              values_from = this_vertex_type,
              names_prefix = "i") %>%
...
print("For SubCortical Cortical")
print(str_c("pvertex: ",calc_DX_reproducible_edgewise("pvertex")))
print(str_c("tvertex: ",calc_DX_reproducible_edgewise("tvertex")))
print(str_c("tvolume: ",calc_DX_reproducible_edgewise("tvolume")))
```
Show in New Window
[1] "For SubCortical Cortical"
[1] "pvertex: 0.76 [0.67; 0.83]*"
[1] "tvertex: 0.74 [0.66; 0.81]*"
[1] "tvolume: 0.72 [0.63; 0.79]*"



```{r}
edgewise_vertex_type_delta_summary <- FC_boots %>%
  semi_join(annotated_graph_edges %>% filter(from_to_type != "SubCort_SubCort")) %>%
  ungroup() %>%
  select(to, from, pvertex:pvertex_minus_tvolume) %>%
  pivot_longer(pvertex:pvertex_minus_tvolume,
               names_to = "comparison_type",
              values_to = "value") %>%
  ungroup() %>%
  group_by(to, from, comparison_type) %>%
  ...
  mutate(sigrange = ((min < 0 & max < 0 ) | (min > 0 & max > 0)),
         sig99CI = ((lower99 < 0 & upper99 < 0 ) | (lower99 > 0 & upper99 > 0)),
         sig95CI = ((lower95 < 0 & upper95 < 0 ) | (lower95 > 0 & upper95 > 0))) %>%
  select(-lower99, -upper99, -lower95, -upper95) %>%
  pivot_wider(id_cols = c("to", "from"),
              names_from = comparison_type,
              values_from = c(mean_val:sig95CI),
              names_glue = "{comparison_type}_{.value}")

saveRDS(edgewise_vertex_type_delta_summary, file = file.path(output_base, "all_clinicalplusqa_group", "Rdata_cache", "06_wholebrain_FC_PINTcombat_roi_DXboots_summary.rds"))
```

```{r} 
pvertex_sum <- edgewise_vertex_type_delta_summary %>%
  filter(pvertex_sig99CI | tvertex_sig99CI) %>%
  filter(pvertex_minus_tvertex_sig99CI) %>%
  select(to, from, tvertex_mean_val, pvertex_mean_val) %>%
  mutate(pvertex_min_tvertex_abs = abs(pvertex_mean_val) - abs(tvertex_mean_val),
         pvertex_better = pvertex_min_tvertex_abs > 0)

pvertex_sum %>%
  ungroup() %>%
  count(pvertex_better)
```
```{r} 
tvertex_sum <- edgewise_vertex_type_delta_summary %>%
  filter(tvertex_sig99CI | tvolume_sig99CI) %>%
  filter(tvertex_minus_tvolume_sig99CI) %>%
  select(to, from, tvertex_mean_val, tvolume_mean_val, tvertex_minus_tvolume_mean_val) %>%
  mutate(tvertex_min_tvolume_abs = abs(tvertex_mean_val) - abs(tvolume_mean_val),
         tvertex_better = tvertex_min_tvolume_abs > 0)

tvertex_sum %>%
  ungroup() %>%
  count(tvertex_better)
```
```{r}
tvertex_sum %>%
  arrange(tvertex_better)
```
...
```{r}
DX_lm_model_DXcohenD_booted <- DX_lm_model_DXcohenD %>%
  pivot_wider(id_cols = all_of(c("to", "from")), 
              values_from = statistic:cohenD_DX_tstatdir,
              names_from = vertex_type,
              names_glue = "{vertex_type}_{.value}") %>%
  inner_join(edgewise_vertex_type_delta_summary, by = c("to", "from"))

saveRDS(DX_lm_model_DXcohenD_booted, file = file.path(output_base, "all_clinicalplusqa_group", "Rdata_cache", "06_wholebrain_FC_DXlmfits_plusboots.rds"))
```

```{r}
DX_lm_model_DXcohenD_booted %>%
  filter(pvertex_p_FDR < 0.05) %>%
  arrange(desc(pvertex_p.value)) %>%
  slice(1:5)
```
...
```{r eval = FALSE}
DX_lm_model_DXcohenD_booted <- readRDS(file = file.path(output_base, "all_clinicalplusqa_group", "Rdata_cache", "06_wholebrain_FC_DXlmfits_plusboots.rds"))
```

```{r}
DX_lm_model_DXcohenD_booted_long <- DX_lm_model_DXcohenD_booted %>%
  select(!(contains("minus"))) %>%
  pivot_longer(c(-to,-from),
               cols_vary = "slowest",
               names_to = c("vertex_type", ".value"),
               names_sep = "_") %>%
  rename("full_cohenD" = DX, 
         "full_abscohenD" = cohenD,
         "booted_mean_cohenD" = mean,
         "p_FDR" = p,
         "booted_min_cohenD" = min,
         "botted_max_cohenD" = max)
...
## significant edges looking using whole sample

```{r}
DX_sig_edge_table <- DX_lm_model_DXcohenD_booted_long %>%
  inner_join(annotated_graph_edges, by = c("to", "from")) %>%
  mutate(sig_edge = if_else(p_FDR < 0.05, "sig", "ns"),
         stat_pos = if_else(statistic > 0, "pos", "neg")) %>%
...
## significant edges looking using bootstrapped sample

```{r}
DX_lm_model_DXcohenD_booted_long %>% names()
```
...
DX_sig_edge_table <- DX_lm_model_DXcohenD_booted_long %>%
  count(vertex_type,  sig99CI) %>%
  spread(sig99CI, n) %>%
  mutate(perc_sig = `TRUE`/(`TRUE` + `FALSE`)*100)
...
```{r}
pvertex_sum <- DX_lm_model_DXcohenD_booted %>%
  filter(tvertex_p_FDR < 0.05 | pvertex_p_FDR < 0.05) %>%
  filter(pvertex_minus_tvertex_sig95CI) %>%
  select(to, from, tvertex_mean_val, pvertex_mean_val) %>%
  mutate(pvertex_min_tvertex_abs = abs(pvertex_mean_val) - abs(tvertex_mean_val),
         pvertex_better = pvertex_min_tvertex_abs > 0) 
...
```
```{r}
tvertex_sum <- DX_lm_model_DXcohenD_booted %>%
  filter(tvertex_p_FDR < 0.05 | tvolume_p_FDR < 0.05) %>%
  filter(tvertex_minus_tvolume_sig95CI) %>%
  select(to, from, tvertex_mean_val, tvolume_mean_val) %>%
  mutate(tvertex_min_tvolume_abs = abs(tvertex_mean_val) - abs(tvolume_mean_val),
         tvertex_better = tvertex_min_tvolume_abs > 0) 
...
```{r}
DX_lm_model_DXcohenD_booted %>%
  filter(p_FDR_tvertex < 0.05 | p_FDR_tvolume < 0.05) %>%
  
```

```{r}
the_sig_to_really_check <- DX_lm_model_DXcohenD %>%
  
  inner_join(annotated_graph_edges, by = c("to", "from")) %>%
  mutate(sig_edge = if_else(p_FDR < 0.05, "sig", "ns")) %>%
  filter(term == "DXSSD", from_to_type != "SubCort_SubCort", sig_edge=="sig") %>%
  select(to, from) %>%
  distinct()

sig_edges_merged <- the_sig_to_really_check %>% 
  inner_join(DX_lm_model_DXcohenD, by = c("term", "to", "from")) %>%
  pivot_wider(id_cols = c("term", "to", "from"),
              names_from = "vertex_type",
              values_from = statistic:cohenD_DX_tstatdir) %>%
  inner_join(edgewise_vertex_type_delta_summary, by = c("to", "from")) 
...
```{r}
sig_edges_merged %>%
  ggplot(aes(x = DX_cohenD_tvolume, y = mean_val_tvolume)) +
  geom_point()
```
...
```{r}
DX_lm_model_DXcohenD_booted_long %>% 
  ggplot(aes(x = full_cohenD, y = booted_mean_cohenD)) +
  geom_point()
```
...
```
```{r}
annotated_graph_edges %>% filter(from_to_type=="SubCort_Cort") %>% unique()
```
...
if (no_ticks==TRUE) {
  ...
  full_plt <- plot_grid(title, DM_DA, DM_VA, DA_SM, VI_VI,
                   ncol = 1, rel_heights = c(0.5, 1,  1, 1, 1, 1,1.5))
  return(full_plt)
}
...
fig1 <- fplot %>%
  ggplot(aes(y = striatum_CortFP, x = DX, color = DX)) + 
...
```{r, fig.height=4, fig.width=6}
fig1 <- fplot %>%
  ggplot(aes(y = thalamus_CortVI, x = DX, color = DX)) + 
  geom_boxplot(outlier.shape = NA) + 
  geom_dotplot(binaxis = 'y', stackdir = 'center', binwidth = 0.01, alpha = 0.5) +
  scale_color_manual(values = c("grey20","red")) +
  facet_wrap( ~ Site, ncol = 4) +
...
networkg_order = c("CortDM","CortFP", "CortVA","CortDA","CortSM","CortVI", 
...
## had to split this by vertex because of RAM issues on laptop
pvertex_networkwise <- results_pheno %>%
  filter(vertex_type == "pvertex") %>%
  networkwise_corrz()
tvertex_networkwise <- results_pheno %>%
  filter(vertex_type == "tvertex") %>%
  networkwise_corrz()
tvolume_networkwise <- results_pheno %>%
  filter(vertex_type == "tvolume") %>%
  networkwise_corrz()
...
```{r}
saveRDS(networkwise_corrs, file = file.path(output_base, "all_clinicalplusqa_group", "Rdata_cache", "06_wholebrain_FC_PINTcombat_networkwise_cache.rds"))
```

```{r}
networkwise_corrs <- readRDS(file = file.path(output_base, "all_clinicalplusqa_group", "Rdata_cache", "06_wholebrain_FC_PINTcombat_networkwise_cache.rds"))
```
...
```{r}
DX_effectednets <- DX_lm_model_DXcohenD_networkwise %>%
   filter(p_FDR < 0.05) %>%
   select(to, from, vertex_type) %>%
  distinct() %>%
   count(to, from) %>%
  filter(n >1)

write_csv(DX_effectednets, file = file.path(output_base, "all_clinicalplusqa_group", "Rdata_cache", "06_PINTcombat_networkwise_signetworks_cache.csv"))
...
DX_lm_model_DXcohenD_networkwise %>%
  mutate(to = factor(to, levels = rev(networkg_order)),
         ...
         corrtype = factor(vertex_type, levels = rev(c('pvertex', 'tvertex', 'tvolume')),
                      labels = rev(c("Surface Personalized", "Surface Template", "Volume Template"))),
         sig = if_else(p_FDR < 0.05, "*","")) %>%
  ggplot(aes(x = to, y = from, fill = DX_cohenD)) +
  geom_tile() +
  geom_text(aes(label = sig)) +
  scale_fill_distiller(breaks = c(-1,1), type = "div", palette = 5, trans = "reverse") +
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  facet_wrap(~ corrtype) +
  labs(x = NULL, y = NULL)
...
```{r}
saveRDS(networkwise_boot_cohenDDX_results, file = file.path(output_base, "all_clinicalplusqa_group", "Rdata_cache", "06_wholebrain_FC_PINTcombat_networkwise_DXboots_cache.rds"))
```

```{r}
networkwise_boot_cohenDDX_results <- readRDS(file = file.path(output_base, "all_clinicalplusqa_group", "Rdata_cache", "06_wholebrain_FC_PINTcombat_networkwise_DXboots_cache.rds"))
```

### building tables of the DX boots
...
networkwise_boot_cohenDDX_results %>% 
  pivot_longer(pvertex:pvertex_minus_tvolume,
               names_to = "colname",
               values_to = "D") %>%
  group_by(to, from, colname) %>%
  summarise(ci_str = calculate_mean_ci(D)) %>%
  pivot_wider(names_from = colname,
              values_from = ci_str) %>%
  select(to, from, tvolume, tvertex, pvertex, tvertex_minus_tvolume, pvertex_minus_tvertex) %>%
  knitr::kable(col.names = c("","","Volume Template", "Surface Template", "Surface Personalized", "Surface-Volume", "Personalized-Template"))
  
```

```{r}
calc_DX_reproducible <- function(this_vertex_type) {
matrix_data <- networkwise_boot_cohenDDX_results %>%
  mutate(pnum = rep(seq(from = 1, to = 1000), each = 45)) %>%
  pivot_wider(id_cols = c("to", "from"),
              names_from = "pnum",
              values_from = this_vertex_type,
              names_prefix = "pnum") %>%
  select(starts_with("pnum")) %>%
  cor() 
...
print(str_c("pvertex: ",calc_DX_reproducible("pvertex")))
print(str_c("tvertex: ",calc_DX_reproducible("tvertex")))
print(str_c("tvolume: ",calc_DX_reproducible("tvolume")))

```

```{r , fig.height=4, fig.width = 5}
library(cowplot)
fplot <- results_net_pheno %>%
  ungroup() %>%
  filter(vertex_type == "pvertex") %>%
  unite(edge_group, to:from) %>%
  unite(dat_sub, dataset, subject) %>%  
  select(dat_sub, Site, DX, edge_group, weight) %>%
  spread(edge_group, weight) 

```

```{r, fig.width=5, fig.height = 4}
ggdraw(plot_DX_scatter_elipsed(fplot, x = thalamus_CortSM, y = striatum_CortFP))
```

```{r}
FC_gscores_lmfitbySite <- results_net_pheno %>%
  group_by(to, from, vertex_type, Site) %>%
  do(tidy(lm(weight ~ DX + Age_match_pt + Sex + fd_mean_match_pt,.))) 

FC_gscores_lmfitbySite %>%
  select(to, from, Site, vertex_type, term, statistic, p.value) %>%
  filter(term == "DXSSD") %>%
  knitr::kable()

FC_gscores_lmfitbySite %>% 
  mutate(node_class = if_else(str_detect(to,'Cort'), 
                 "Within_Cortical",
                 "Subcortical_to")) %>%
  filter(term == "DXSSD",
         node_class == "Subcortical_to") %>%
  ggplot(aes(y = abs(statistic), x = rev(vertex_type))) +
    geom_point() +
    geom_line(aes(group = to)) +
  facet_wrap(~Site)
```
