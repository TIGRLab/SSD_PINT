...
```{r}
focus_results %>%
  mutate(pval_str = ifelse(p.value < 0.001, 
                           format(p.value, scientific = TRUE, digits = 2),
                           format(p.value, digits = 3, nsmall = 3))) %>%
  mutate(report_str = str_c("d = ", format(cohenD, digits = 3, nsmall = 1),
                            ", t(",parameter, ') =', 
                            format(statistic, digits = 3, nsmall = 0),
                            ', p=', pval_str)) %>%
  ...
  arrange(test_vars, subcort_ROI, YeoNet) %>%
  knitr::kable()
```
