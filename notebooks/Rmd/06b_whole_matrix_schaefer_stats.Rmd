...
results_pheno_plus_DXd <- function(results_pheno) {
  results_pheno %>%
  semi_join(Schaefer_node_annots, by = c("to"="node_name")) %>%
  semi_join(Schaefer_node_annots, by = c("from"="node_name")) %>%
  mutate(corZ = weight) %>%
...
annotated_graph_edges <- DX_lm_model_full %>%
  ...
  select(to, from) %>%
  distinct() %>%
  inner_join(Schaefer_node_annots, by = c("to"="node_name")) %>%
  inner_join(Schaefer_node_annots, by = c("from"="node_name"), suffix = c('_to','_from')) %>%
...
```{r}
saveRDS(DX_lm_model_DXcohenD, file = file.path(output_base, "all_clinicalplusqa_group", "Rdata_cache", "06_wholebrain_FC_DXlmfits_Schaefer.rds"))
```

can get the model fit back from here..

```{r eval = FALSE}
DX_lm_model_DXcohenD <- readRDS(file = file.path(output_base, "all_clinicalplusqa_group", "Rdata_cache", "06_wholebrain_FC_DXlmfits_Schaefer.rds"))
```
...
# cd ~/code/SSD_PINT
sbatch --array=0-99 ./code/bin/submit_SchaeferFC_DXeffect_10permutations.sh
```

```{r}
FC_boots <- list.files(path = file.path(output_base, "all_clinicalplusqa_group", "Rdata_cache","06b_SchaeferFC_boots"), pattern = "*.rds", full.names = TRUE ) %>%
  map_dfr(readRDS)
```
...
print(str_c("surfschaefer: ",calc_DX_reproducible_edgewise("surfschaefer")))
print(str_c("volschaefer: ",calc_DX_reproducible_edgewise("volschaefer")))
```
```{r}
...
edgewise_vertex_type_delta_summary <- FC_boots %>%
  ...
  ungroup() %>%
  select(to, from, surfschaefer:surf_minus_vol) %>%
  pivot_longer(surfschaefer:surf_minus_vol,
...
saveRDS(edgewise_vertex_type_delta_summary, file = file.path(output_base, "all_clinicalplusqa_group", "Rdata_cache", "06_wholebrain_FC_Schaefercombat_roi_DXboots_summary.rds"))
```
```{r} 
tvertex_sum <- edgewise_vertex_type_delta_summary %>%
  filter(surfschaefer_sig99CI | volschaefer_sig99CI) %>%
  filter(surf_minus_vol_sig99CI) %>%
  select(to, from, surfschaefer_mean_val, volschaefer_mean_val, surf_minus_vol_mean_val) %>%
  mutate(tvertex_min_tvolume_abs = abs(surfschaefer_mean_val) - abs(volschaefer_mean_val),
...
saveRDS(DX_lm_model_DXcohenD_booted, file = file.path(output_base, "all_clinicalplusqa_group", "Rdata_cache", "06_wholebrain_FC_DXlmfits_plusboots_Schaefer.rds"))
```

```{r}
DX_lm_model_DXcohenD_booted %>%
  filter(surfschaefer_p_FDR < 0.05) %>%
  arrange(desc(surfschaefer_p.value)) %>%
  slice(1:5)
```
...
tvertex_sum <- DX_lm_model_DXcohenD_booted %>%
  filter(surfschaefer_p_FDR < 0.05 | volschaefer_p_FDR < 0.05) %>%
  filter(surf_minus_vol_sig95CI) %>%
  select(to, from, surfschaefer_mean_val, volschaefer_mean_val) %>%
  mutate(tvertex_min_tvolume_abs = abs(surfschaefer_mean_val) - abs(volschaefer_mean_val),
         tvertex_better = tvertex_min_tvolume_abs > 0) 
...
DX_lm_model_DXcohenD %>%
  ...
  make_swirly_results_plot(pos_label = "SSD > HC", 
                      neg_label = "HC > SSD",
                      plot_title = "Schaefer100 Surface",
                      node_annotations = Schaefer_node_annots)
```
...
node_groupings <- factor(1:9, 
                         levels = c(1:9),
                         labels = c(levels(Schaefer_node_annots$subcort_ROI),
...
DX_lm_model_DXcohenD %>%
  ...
  unnest(uptri) %>%
  mutate(to_lab = factor(to, levels = (Schaefer_node_annots %>% arrange(subcort_ROI, network))$node_name),
         from_lab = factor(from, levels = (Schaefer_node_annots %>% arrange(subcort_ROI, network))$node_name)) %>%
...
full_DX_mat <- DX_lm_model_DXcohenD %>%
  filter(term == "DXSSD", vertex_type == "surfschaefer") %>%
  ungroup() %>%
  select(to, from, statistic) %>%
  uppertri_df_to_full() 

full_DX_mat %>%
  mutate(to_lab = factor(to, levels = Schaefer_node_annots$node_name), 
         from_lab = factor(from, levels = Schaefer_node_annots$node_name)) %>%
  ggplot(aes(x=to_lab, y=from_lab, fill=value)) + 
  geom_tile(color = "white") +
  scale_fill_gradient2(high = "#b2182b", mid = "white", low = "#2166ac") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
get_subcort_net_edges <- function(data, this_YeoNet, D_threshold) {
      ...
      uppertri_df_to_full() %>%
      inner_join(Schaefer_node_annots, by = c("to"="node_name")) %>%
      inner_join(Schaefer_node_annots, by = c("from"="node_name")) %>%
...
DXweigths_tvolume <- data %>%
  filter(term == "DXSSD", vertex_type == "volschaefer") %>%
  ungroup() %>%
  get_subcort_net_edges(.,this_YeoNet)

Fz_DXweigths <- bind_rows(tvertex = DXweigths_tvertex,
                     tvolume = DXweigths_tvolume,
                     .id = "vertex_type") %>%
      mutate(corrtype = factor(vertex_type, levels = c('tvertex', 'tvolume'),
                             labels = c("Surface Template", 
                                        "Volume Template")),
