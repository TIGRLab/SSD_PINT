...
if [[ ${session} = *"ses"* ]]; then
    ...
    ${ciftify_container} ciftify_clean_img \
        ...
        /archiveout/ciftify/${subject}/MNINonLinear/Results/${session}_${func_base}/${session}_${func_base}_Atlas_s0.dtseries.nii
else
  singularity exec \
      mkdir -p ${outdir}/ciftify_clean_img/${subject}
      singularity exec \
      ...
      ${ciftify_container} ciftify_clean_img \
          ...
          --left-surface=/archiveout/ciftify/${subject}/MNINonLinear/fsaverage_LR32k/${subject}.L.midthickness.32k_fs_LR.surf.gii \
          --right-surface=/archiveout/ciftify/${subject}/MNINonLinear/fsaverage_LR32k/${subject}.R.midthickness.32k_fs_LR.surf.gii \
          /archiveout/ciftify/${subject}/MNINonLinear/Results/${func_base}/${func_base}_Atlas_s0.dtseries.nii
fi
...
if [[ ${session} = *"ses"* ]]; then
    ...
    -B ${archive_pipedir}:/archiveout \
    ${ciftify_container} ciftify_clean_img \
        --output-file=/output/ciftify_clean_img/${subject}/${session}/${subject}_${session}_${func_base}_desc-cleansm0_bold.dtseries.nii \
  ...
  singularity exec \
      ...
      -B ${archive_pipedir}:/archiveout \
      ${ciftify_container} ciftify_clean_img \
          --output-file=/output/ciftify_clean_img/${subject}/${subject}_${func_base}_desc-cleansm0_bold.dtseries.nii \
          --clean-config=/output/ciftify_clean_img/24MP_8Phys_4GSR_sm0.json \
          --confounds-tsv=/archiveout/fmriprep/${subject}/func/${subject}_${func_base}_confounds.tsv \
          /archiveout/ciftify/${subject}/MNINonLinear/Results/${func_base}/${func_base}_Atlas_s0.dtseries.nii
fi
...
# Step 2. Extract the subcortical timeseries from the cleaned files
