...
# edited: 2017/11/23 -- by edickie for COBRE dataset

set -e

export DIR_MODULES=/archive/code/epitome
export DIR_DATA=/external/SchizConnect/COBRE/epitome
export DIR_EXPT=COBRE
...
for SESS in $(ls -d -- ${DIR_DATA}/${DIR_EXPT}/${SUB}/T1/*/); do
    ...
    sessname=$(basename ${SESS})
    subjid="${DIR_EXPT}_${SUB}_${sessname}"
    if [ ! -f ${HCP_DATA}/${subjid}/MNINonLinear/fsaverage_LR32k/${subjid}.thickness.32k_fs_LR.dscalar.nii ]; then
        ciftify_recon_all ${subjid}
        cifti_vis_recon_all snaps ${subjid}
    fi
done
...
for SESS in ${DIR_SESS}; do
    ...
    for RUN in ${DIR_RUNS}; do
        ...
        if [ ! -f ${HCP_DATA}/${subjid}/MNINonLinear/Results/${DATA_TYPE}_${NUM}/${DATA_TYPE}_${NUM}_Atlas_s${fwhm}.dtseries.nii ]; then
            ciftify_subject_fmri \
                --SmoothingFWHM ${fwhm_hcp} \
                ${SESS}/${input}.${ID}.${NUM}.nii.gz \
                ${subjid} \
                ${DATA_TYPE}_${NUM}
            cifti_vis_fmri snaps \
              --SmoothingFWHM ${fwhm_hcp} \
              ${DATA_TYPE}_${NUM} \
              ${subjid}
