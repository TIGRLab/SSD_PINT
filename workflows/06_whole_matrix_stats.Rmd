...
save(all_subcort_results, file = "../intermediate/06_wholebrain_results_cache.Rdata")
```

```{r load-cached-results, eval=FALSE}
load("../intermediate/06_wholebrain_results_cache.Rdata")
```
...
Fz_DXweigths <- bind_rows(pvertex = Fz_DXweigths_pvertex,
                     ...
                     .id = "vertex_type") 
Fz_DXweigths %>% 
  group_by(vertex_type, FC_subtype) %>%
  summarise(median_weight = median(edge_FC_weight)) %>%
  ggplot(aes(y = abs(median_weight), x = rev(vertex_type), color = FC_subtype)) +
    geom_point() +
    geom_line(aes(group = FC_subtype))
  
```
```{r}
Fz_DXweigths %>% 
  count(FC_subtype, vertex_type) %>%
...
c(levels(node_annotations$subcort_ROI),
...
FC_scores_lmfit %>% 
  group_by(vertex_type, FC_subtype) %>%
  filter(term == "DXSSD",
         str_detect(FC_subtype,"subcort")) %>%
  ggplot(aes(y = abs(statistic), x = rev(vertex_type), color = FC_subtype)) +
    geom_point() +
    geom_line(aes(group = FC_subtype))
```
```{r}
FC_scores_lmfit <- FC_scores_together %>%
  inner_join(pheno, by = c("subject", "dataset")) %>%
  ungroup() %>%
  group_by(FC_subtype, vertex_type, Site) %>%
  do(tidy(lm(mwcorZ ~ DX + Age_pt + Sex + fd_mean_pt,.))) 

FC_scores_lmfit %>%
  select(FC_subtype, vertex_type, Site, term, statistic, p.value) %>%
  filter(term == "DXSSD") %>%
  knitr::kable()
...
SSD_cors <- FC_scores_together %>%
  ...
  select(subject, dataset, FC_subtype, mwcorZ) %>%
  spread(FC_subtype, mwcorZ) %>%
  select(-subject) %>%
  cor(.) 
...
write_csv(FC_scores_together, '../data/intermediate/SSD_DX_subject_scores/SSD4cohorts_DXweighted_moregroups_subject_scores.csv')
```
